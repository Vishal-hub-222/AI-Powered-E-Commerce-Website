...
export const AiAssistantDrawer = ({ isOpen, onClose, onQuickViewProduct }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/70 backdrop-blur-sm animate-fade-in">
      <div className="w-full sm:max-w-lg h-full glass-panel-glow sm:border-l border-indigo-500/30 flex flex-col justify-between shadow-2xl animate-slide-left">
        {/* Header */}
        <div className="p-3.5 sm:p-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/80">
          <div className="flex items-center gap-2.5 sm:gap-3">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl gradient-btn flex items-center justify-center shadow-lg shadow-indigo-500/30 relative">
              <Bot className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
              <span className="absolute -bottom-0.5 -right-0.5 w-2.5 sm:w-3 h-2.5 sm:h-3 bg-emerald-400 rounded-full border-2 border-slate-900" />
            </div>
            <div>
              <div className="flex items-center gap-1.5">
                <h3 className="font-bold text-white font-display text-sm sm:text-base">Aura AI Concierge</h3>
                <span className="px-1.5 py-0.5 rounded text-[9px] sm:text-[10px] font-bold bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">
                  AI v2.5
                </span>
              </div>
        ...
        <div className="p-2.5 sm:p-3 border-t border-slate-800/80 bg-slate-950/60 overflow-x-auto no-scrollbar touch-scroll flex gap-2">
          {quickPrompts.map((prompt, i) => (
            <button
              key={i}
              onClick={() => handleSendMessage(prompt)}
              className="px-2.5 py-1 rounded-full bg-slate-900 hover:bg-slate-800 text-[10px] sm:text-[11px] text-slate-300 hover:text-white border border-slate-800 shrink-0 transition-colors whitespace-nowrap"
            >
              {prompt}
            </button>
          ))}
        ...
        <form
          ...
          className="p-3 sm:p-4 border-t border-slate-800 bg-slate-950"
        >
          <div className="relative flex items-center">
            <input
              ...
              className="w-full bg-slate-900 text-xs text-slate-100 placeholder-slate-400 pl-3.5 sm:pl-4 pr-12 py-2.5 sm:py-3 rounded-xl border border-slate-700/70 focus:outline-none focus:border-indigo-500 shadow-inner"
            />
            <button
              type="submit"
